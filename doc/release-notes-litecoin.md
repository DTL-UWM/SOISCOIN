Soiscoin Core version *0.15.1* is now available from:

  <https://download.soiscoin.org/soiscoin-0.15.1/>
...
A number of changes to the way Soiscoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 soislites per kB (0.00001 SOIS). This is to prepare for dropping the minimum transaction fee to 0.1 soislites per kB (0.0001 SOIS) in the next release.
